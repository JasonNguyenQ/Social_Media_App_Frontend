...
export default function Messages(){
    ...
    function HandleScroll(){
        ...
        for(let i = 0; i < length; i++){
            ...
            childHeights += child.clientHeight + parseFloat(getComputedStyle(child).marginBottom)
        }
        ...
        if(message.from === username){
            return (
                <div key={index} className="message self-message">
                    <p className="message-header">{message.from} | {date}</p>
        ...
        <>
            ...
            <div className="container">
                <div className="threads-container">
                    {threads.map((thread: ThreadInfo)=>{
                        return (
                            <div 
                                key={thread.threadId} 
                                className={`thread ${(activeThread === thread.threadId) ? "active-thread" : ""}`} 
                                onClick={()=>{ThreadHandler(thread)}}>
                                {thread.threadName}
                            </div>
                        );
                    })}
                ...
                <div className="message-container">
                    <div className="messages" ref={messageContainer} onScroll={HandleScroll}>
                        {messages.length > 0 && messages.map((message, index) => CreateMessage(index, message))}
                    </div>
                    ...
                    <button hidden={!viewOlder} onClick={scrollToLastMessage}>View Recent...</button>
                </div>
            </div>
        </>
    )
